...
class Helper
{
    ...
    public static function get_data($param, $limit = 99999999)
    {
        $data = DB::table($param)->limit($limit)->get();
        return isset($data) ? $data : null;
    }

    public static function get_data_join($param, $join)
    {
        $data = DB::table($param)->join($join . 's', $param . '.' . $join . '_id', '=', $join . 's.id')->select($param . '.*', $param . '.id as id_' . $param, $join . 's.id as id_' . $join, $join . 's.*')->get();
        return isset($data) ? $data : null;
    }
    ...
    public static function arrayKeyToString($param)
    {
        $data = null;
        $keys = array_keys($param);
        for ($i = 0; $i < count($param); $i++) {
            if ($data == null) {
                $data = $keys[$i] . " : " . $param[$keys[$i]] . " <br>";
            } else {
                $data = $data . "" . $keys[$i] . " : " . $param[$keys[$i]] . " <br>";
            }
        }
        return $data;
    }
    ...
    public static function getImagePetani($id)
    {
        $data = ['1.jpg', '2.jpg', '3.jpg', '4.webp', '5.jpg', '6.jpg', '7.jpg'];
        return $data[$id % 7];
    }

    public static function cuacaToValue($cuaca)
    {
        switch ($cuaca) {
            case "panas":
                $cuaca = 1;
        ...
        return isset($cuaca) ? $cuaca : null;
    }

    public static function ifDevisionByZero($num1, $num2)
    {
        if ($num2 == 0) {
            return 0;
        } else {
